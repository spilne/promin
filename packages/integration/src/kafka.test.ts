...
// ---------------------------------------------------------------------------
// Helper — create topic via admin (Kafka auto-create is slow)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// KafkaTopic — publish / subscribe
// ---------------------------------------------------------------------------

withKafka("KafkaTopic — publish and subscribe via kafkajs", (ctx) => {
...
// ---------------------------------------------------------------------------
// OffsetTracker — already unit-tested, but verify with real offsets
// ---------------------------------------------------------------------------

withKafka("OffsetTracker — contiguous commit tracking", (ctx) => {
  it("only commits contiguous offsets after parallel processing", async () => {
    ...
    tracker.complete(0, 2); // offset 2 done first
    tracker.complete(0, 0); // offset 0 done
    // offset 1 still pending

    const committable = tracker.committable();
    // Can only commit up to offset 1 (0 is done, but 1 is missing)
    expect(committable.get(0)).toBe(1);

    // Now complete offset 1
    tracker.complete(0, 1);
    const committable2 = tracker.committable();
    // Now can commit up to offset 3 (0, 1, 2 all done)
    expect(committable2.get(0)).toBe(3);
  });
});

// ---------------------------------------------------------------------------
// Consumer groups — two consumers share a topic
// ---------------------------------------------------------------------------

withKafka("Consumer groups — multiple consumers share partitions", (ctx) => {
  it("two consumers in same group each get a subset of messages", async () => {
    const topic = uniqueName("cg");
    await createTopic(ctx.broker, topic, 2); // 2 partitions
    ...
    // Two consumers — each gets some messages
    ...
    await Promise.race([
      Promise.all([p1, p2]),
      new Promise((r) => setTimeout(r, 15_000)), // safety timeout
    ]);

    // Together they consumed all 10
