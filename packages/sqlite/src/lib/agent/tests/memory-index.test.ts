...
memoryIndexTestSuite(makeStore);
...
describe("SqliteMemoryIndex", () => {
  it("persists across instances sharing the same db", async () => {
    ...
    const s1 = SqliteMemoryIndex.make({ db });
    const id = await s1.save({ content: "persisted across instances" });

    const s2 = SqliteMemoryIndex.make({ db });
  ...
  it("custom table name avoids conflicts", async () => {
    const db = new Database(":memory:");
    const a = SqliteMemoryIndex.make({ db, table: "mem_a" });
    const b = SqliteMemoryIndex.make({ db, table: "mem_b" });
  ...
  it("store-level namespace isolates entries from unscoped stores", async () => {
    const db = new Database(":memory:");
    const global = SqliteMemoryIndex.make({ db });
    const ns = SqliteMemoryIndex.make({ db, namespace: "agent-1" });
  ...
  it("two stores with different namespaces share one table without interference", async () => {
    const db = new Database(":memory:");
    const a = SqliteMemoryIndex.make({ db, namespace: "agent-a" });
    const b = SqliteMemoryIndex.make({ db, namespace: "agent-b" });
  ...
  it("explicit per-call scope overrides store-level namespace", async () => {
    const db = new Database(":memory:");
    const s = SqliteMemoryIndex.make({ db, namespace: "agent-1" });
  ...
  it("store-level namespace applies to search", async () => {
    const db = new Database(":memory:");
    const a = SqliteMemoryIndex.make({ db, namespace: "ns-a" });
    const b = SqliteMemoryIndex.make({ db, namespace: "ns-b" });
